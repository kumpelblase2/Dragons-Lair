...
public enum EventActionOptions
{
	NPC_SPAWN(
			...
			new String[0]),
	BLOCK_CHANGE(
			new String[] { "world", "block_id", "x", "y", "z" },
			new String[] { "x2", "y2", "z2" }),
	MOB_SPAWN(
			new String[] { "world", "mob_id", "x", "y", "z" },
			new String[] { "amount" }),
	OBJECTIVE_COMPLETE(
	...
	ITEM_REMOVE(
			...
			new String[] { "scope", "on_failure", "on_success" }),
	PLAYER_WARP(
	...
	DUNGEON_END(
			new String[] { "dungeon_id" },
			new String[] { "warp_on_end", "give_items" }),
	DUNGEON_REGISTER(
			new String[] { "dungeon_id" },
			new String[0]),
	ITEM_ADD(
			new String[] { "item_id", "amount" },
			new String[] { "scope", "damage" }),
	CHAPTER_COMPLETE(
	...
	NPC_STOP_ATTACK(
			...
			new String[0]),
	NPC_WALK(
			new String[] { "npc_id", "x", "y", "z" },
			new String[0]),
	NPC_DIALOG(
			new String[] { "npc_id", "dialog_id" },
			new String[] { "send_to" }),
	ITEM_SPAWN(
			new String[] { "world", "x", "y", "z", "item_id" },
			new String[] { "amount" }),
	BROADCAST_MESSAGE(
			new String[] { "message" },
			new String[] { "permission", "dungeon_id" }),
	SAY(
			new String[] { "message" },
			new String[0]),
	ADD_POTION_EFFECT(
			new String[] { "potiontype", "amplifier", "duration" },
			new String[] { "scope" }),
	REMOVE_POTION_EFFECT(
			new String[] { "potiontype" },
			new String[] { "duration", "scope" }),
	CHANGE_LEVEL(
			new String[] { "amount", "change_type" },
			new String[] { "scope" }),
	CHANGE_HEALTH(
			new String[] { "amount", "change_type" },
			new String[] { "scope" }),
	CHANGE_HUNGER(
			new String[] { "amout", "change_type" },
			new String[] { "scope" }),
	EXECUTE_COMMAND(
			new String[] { "command" },
			new String[] { "execute_as" }),
	...
	public static EventActionOptions byType(final EventActionType type)
	{
	...
	{
		for(final String general : this.generalOptions)
			if(general.equals(option))
				return true;
		for(final String required : this.requiredOptions)
			if(required.equals(option))
				return true;
		for(final String optional : this.optionalOptions)
